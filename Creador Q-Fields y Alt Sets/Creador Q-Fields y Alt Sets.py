        ...
        try:
                ...
                try:
                        ...
                        if(banderaAltSet == 1):
                                ...
                                row += 1
                                column = 0
                                headers = ["# Key","Parent","In survey","In mobile survey","In report","Short form","Description","Visibility","SequenceNumber","NumericValue","Export value","PriorityRaw","RIColumn","RIColSpan","BoxColor","FontColor","Is Other Option","TranslationExplanation","uuid"]
                                for item in headers:
                                        aWorksheet.write(row, column, item)
                                        column += 1
                                #Creación AltDbs
                                        ...
                                        if(worksheet.cell_type(curr_row, 8) in (xlrd.XL_CELL_EMPTY, xlrd.XL_CELL_BLANK) and worksheet.cell_type(curr_row, 1) not in (xlrd.XL_CELL_EMPTY, xlrd.XL_CELL_BLANK)):
                                                ...
                                                for altdb in altdbArray:
                                                        ...
                                                        if(altdbLabel != -1):
                                                                ...
                                                                if(altdb.isnumeric()):
                                                                        numericValue = altdb
                                                        elements = [nombrePadre,inSurveyAltdb,inMobileSurveyAltdb,inReport,shortForm,descriptionAltdb,"SURVEY_AND_REPORTING_REQUIRED",contadorHijo,numericValue,"","","","","","",isOther,"",""]
                                                        for item in elements:
                                                                aWorksheet.write(row, column, item)
                                                                column += 1
                                                        column = 1
                        ...
                        while curr_row < num_rows:
                                ...
                                curr_row += 1
                        qWorkbook.close()
                        sg.popup('Resultados: ','\nSe han generado tus archivos en la misma carpeta en la que se encuentra este programa, si no los ves, refresca la página o revisa que el spec este llenado correctamente. \n\nEn caso de que hayas creado altsets, no olvides llenar el espacio del altset del excel con los valores de los recien creados. \n\nEn caso de que hayas creado una pregunta de Other, recuerda procesar el bulk primero sin el campo que tiene el other question y una vez procesado, procesar unicamente el de other question\n')
                except:
...
layout = [  [sg.Text('Porfavor introduce la dirección del archivo y el nombre de la hoja en los recuadros inferior y posteriormente haz clic en el botón de "Generar"')], 
            [sg.Text('Dirección del archivo con formato "C:\CarpetaProyecto\Spec Encuesta.xls": '), sg.InputText(key='direccion')], 
...
window = sg.Window('Generador QFields y AltSets', layout) 
